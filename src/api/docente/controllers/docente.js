...
module.exports = {
  async findByAttribute(ctx) {
    ...
    const model = strapi.getModel('api::docente.docente');
    
    const entity = await strapi.db.query('api::docente.docente').findOne({ where: { [attribute]: value } });
  ...
  async findAll(ctx) {
    const model = strapi.getModel('api::docente.docente');
    const entities = await strapi.db.query('api::docente.docente').findMany();
